...
async function main() {
  ...
  await up(db.vendor, [
    ...
    { id: 'vnd_empire', name: "Antonio's", legalName: 'Empire Sewing, Inc', role: 'MANUFACTURER',
      ...
      contactName: 'Antonio', contactInfo: '(323) 742-2267', active: true,
    ...
    { id: 'vnd_dye', name: 'LA Dye Masters', role: 'DYE_HOUSE',
      ...
      contactName: 'Martin', contactInfo: '(213) 354-4186', leadTimeDays: 14, active: true,
      notes: 'Garment dyeing for knits, currently 1-2 weeks. 14 days recorded as the safe planning figure. This is a SEPARATE stage that adds to the manufacturer time — tee goes maker → dye house → back for buttons and finishing.' },
  ...
  await up(db.component, [
    { id: 'cmp_fine_rib', stockedInStudio: false, name: 'Fine rib cotton', category: 'MATERIAL', vendorId: 'vnd_richline',
      ...
      notes: 'In stock at RichLine, so effectively no lead time on a PO. $3.15/yd is already the volume price. An upgrade on the old California Textile Group rib rather than a spec match. Rolls vary between 70 and 80 yards (Michael Pollack) — 75 is recorded as the planning midpoint, but an order of N rolls yields somewhere in a +/- 7% band, so never promise exact yardage.' },
    ...
    { id: 'cmp_leather', stockedInStudio: false, name: 'Leather hide', category: 'MATERIAL', vendorId: 'vnd_leather',
      unitOfMeasure: 'hide', locationId: 'loc_studio', active: true },
    { id: 'cmp_suede', stockedInStudio: false, name: 'Tennessee chocolate suede', category: 'MATERIAL', vendorId: 'vnd_leather',
      unitOfMeasure: 'hide', locationId: 'loc_studio', active: true, notes: 'Bateau handles.' },
    { id: 'cmp_denim', stockedInStudio: false, name: 'Denim', category: 'MATERIAL', vendorId: 'vnd_denim',
      unitOfMeasure: 'yard', locationId: 'loc_studio', active: true },
    { id: 'cmp_canvas', stockedInStudio: false, name: 'Canvas', category: 'MATERIAL', vendorId: 'vnd_denim',
      unitOfMeasure: 'yard', locationId: 'loc_studio', active: true },
    { id: 'cmp_liberty_fabric', stockedInStudio: false, name: 'Liberty print fabric', category: 'MATERIAL', vendorId: 'vnd_liberty',
      unitOfMeasure: 'yard', locationId: 'loc_studio', active: true, notes: 'Betsey and Wiltshire prints. Story Dress.' },
    { id: 'cmp_lining', stockedInStudio: false, name: 'Tibetan cotton lining', category: 'MATERIAL', vendorId: 'vnd_dillu',
      unitOfMeasure: 'yard', locationId: 'loc_studio', active: true },
  ...
  await up(db.actionItem, [
    ...
    { id: 'aq_empire_phone_po', kind: 'QUESTION', entityType: 'VENDOR', entityId: 'vnd_empire',
      ...
      detail: 'Hours are on file (7:30am-4pm, Mon-Fri) and appear on the PO delivery block. The phone number was mentioned but not supplied.',
      source: 'SYSTEM', resolved: false, remindDaysBefore: null },
    { id: 'aq_duplicate_cleo_tee', kind: 'QUESTION', entityType: 'PRODUCT', entityId: 'prd_cleo_tee',
      title: 'Two Cleo Tee listings on Shopify — one is a draft holding 245 units',
      detail: 'Shopify has two separate Cleo Tee products, both Black and White in sizes 1-3. The handle cleo-tee-w is ACTIVE with 99 units. The handle cleo-tee is in DRAFT with 245 units. Draft products cannot be bought, so those 245 units are invisible to customers — roughly $21,560 at retail. Either the draft is an old duplicate whose stock figure is stale, or it is real stock attached to the wrong listing. Needs resolving before any Cleo Tee forecast, because the on-hand count is currently either 99 or 344 depending on which is true.',
      source: 'SYSTEM', resolved: false, remindDaysBefore: null },
